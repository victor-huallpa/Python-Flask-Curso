...
usuarios = [
    ("Ivan", """Nacido en Santiago, Ivan es un apasionado deportista
     que ha representado a su pais en multiples competencias internacionales.
     En su teimpo libre, le gusta explorar la naturaleza y practicar diferentes
      disciplinas deportivas."""),
    ("Pamela", """It is a long established fact that a reader will be distracted by the readable
...
list_users = [user[0] for user in usuarios ]

@app.route("/")
def inicio():
    return render_template(template_name_or_list="index.html", users = list_users)

@app.route("/<username>")
def perfil_user(username):
    user_found = None
